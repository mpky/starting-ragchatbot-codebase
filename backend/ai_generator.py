...
"""
    ...
    def generate_response(self, query: str,
        ...
        if response.stop_reason == "tool_use" and tool_manager:
            return self._handle_sequential_tool_execution(response, api_params, tool_manager, max_rounds=2)
        
        # Return direct response
        return response.content[0].text
    
    def _handle_sequential_tool_execution(self, initial_response, base_params: Dict[str, Any], tool_manager, max_rounds: int = 2) -> str:
        """
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            tool_results, error = self._execute_tools_for_response(current_response, tool_manager)
            ...
            if round_num == max_rounds:
                ...
                final_response = self._make_api_call(messages, base_params["system"], tools=None)
                return self._extract_text_response(final_response)
            else:
                # Continue with tools available for next round
                next_response = self._make_api_call(messages, base_params["system"], base_params.get("tools"))
    ...
    def _handle_tool_execution(self, initial_response, base_params: Dict[str, Any], tool_manager):
        ...
        """
        return self._handle_sequential_tool_execution(initial_response, base_params, tool_manager, max_rounds=1)
    ...
    def _execute_tools_for_response(self, response, tool_manager) -> Tuple[List[Dict], Optional[str]]:
    ...
    def _make_api_call(self, messages: List[Dict], system_prompt: str, tools: Optional[List] = None):
