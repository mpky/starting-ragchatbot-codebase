...
class TestAIGenerator:
    ...
    def test_generate_response_with_tool_use(self):
        ...
        result = self.ai_generator.generate_response(
            "Tell me about Python", tools=mock_tools, tool_manager=mock_tool_manager
        )
    ...
    def test_generate_response_tool_execution_flow(self):
        ...
        final_response = MockResponse(
            "Python is a high-level programming language.", stop_reason="end_turn"
        )
        ...
        mock_tool_manager.execute_tool.assert_called_once_with(
            "search_course_content", query="what is Python", course_name="Python Basics"
        )
        ...
        assert (
            "tools" in second_call_args or "tools" not in second_call_args
    ...
    def test_sequential_tool_calling_max_rounds_limit(self):
        ...
        second_response = MockResponse(
            [tool_content], stop_reason="tool_use"
        )  # Would trigger 3rd round
    ...
    def test_generate_response_multiple_tool_calls(self):
        ...
        initial_response = MockResponse(
            [tool1_content, tool2_content], stop_reason="tool_use"
        )
